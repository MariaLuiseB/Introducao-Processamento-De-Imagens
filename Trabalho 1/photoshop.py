...
class EditorImagem(tk.Tk):
    def __init__(self, root):
        ...
        self.menu_frame1.grid(row=0, column=0, padx=(0,0), pady=(0,5), sticky="ew", columnspan=2)  
    ...
    def aplicar_filtro_log(self):
        if self.imagem_original is not None:
            ...
            self.imagem_log = c * cv2.log(self.imagem_original.astype(np.float64) + 1.0)
    ...
    def carregar_imagem_referencia(self):
            filepath = filedialog.askopenfilename()
            imagem_referencia = cv2.imread(filepath)

            if imagem_referencia is not None:
                self.imagem_referencia = imagem_referencia
                self.hist_especializado(imagem_referencia)
                self.redimensionar_imagem(self.imagem_original)
                # Exiba a imagem de referência ou faça qualquer outra ação necessária
                # Você pode adicionar um widget de visualização da imagem de referência aqui
